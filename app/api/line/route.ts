...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const event of events) {
      ...
      // ผู้ใช้รอกรอกชื่อ
      // ผู้ใช้รอกรอกชื่อ / ผู้ใช้ใหม่
      // ผู้ใช้ใหม่ → รอกรอกชื่อ
      // ตรวจสอบว่าผู้ใช้รอกรอกชื่อหรือไม่
      if (user.awaitingName) {
        const name = userMessage || "ไม่มี";

        // อัปเดตชื่อและ awaitingName = false
      ...
      // ผู้ใช้เก่า (มีชื่ออยู่แล้ว) → ต้อนรับเฉพาะครั้งแรกหลัง login
      else if (!user.awaitingName && userMessage.toLowerCase() === "start") {
        await client.replyMessage(event.replyToken, {
          type: "text",
      ...
      // จากนั้นส่งข้อความต่อไป Gemini AI ได้เหมือนเดิม
      ...
      //   // ถ้าเป็นผู้ใช้ใหม่ → ตอบข้อความแนะนำ
      //   await client.replyMessage(event.replyToken, {
      //     type: "text",
      //     text: `ยินดีที่ได้รู้จักครับ คุณ ${name} 😊 ตอนนี้คุณสามารถถามเรื่องเมนูอาหารหรือสุขภาพได้เลยครับ`,
      //   });
