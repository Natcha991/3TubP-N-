  ...
  try {
    ...
    for (const event of events) {
      if (event.type === "message" && event.message.type === "text") {
        const userMessage = event.message.text.trim();
        const userId = event.source.userId!;
        const isGroup = event.source.type === "group"; // 🆕 ตรวจสอบว่ามาจากกลุ่มหรือไม่
        ...
        if (!user) {
          user = await User.create({
            name: isGroup ? "GroupUser" : undefined, // ถ้ามาจากกลุ่มไม่ต้องถามชื่อ
            lineId: userId,
            conversation: [],
            awaitingName: !isGroup, // ถ้าเป็นกลุ่มก็ไม่ต้องรอชื่อ
            awaitingField: !isGroup ? "birthday" : null, // ถ้าเป็นกลุ่มไม่ต้องถามข้อมูล
            isFromGroup: isGroup,
          });
