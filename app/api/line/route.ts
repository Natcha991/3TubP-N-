  ...
  try {
    ...
    for (const event of events) {
      if (event.type === "message" && event.message.type === "text") {
        ...
        // 🧠 ถ้ามีชื่ออยู่แล้ว (ผู้ใช้เก่า) → ทักทายก่อน
        if (user.name && user.name.trim() !== "" && !user.hasGreeted) {
          await client.replyMessage(event.replyToken, {
            type: "text",
            text: `ยินดีต้อนรับคุณ ${user.name} เข้าสู่การให้บริการทาง LINE Chatbot 🩵\nหวังว่าจะสร้างความสะดวกสบายให้กับคุณ และขอขอบคุณสำหรับแรงสนับสนุนตลอดมาครับ 🙏`,
          });

          // ✅ อัปเดตสถานะว่า "ทักแล้ว" (จะไม่ทักซ้ำ)
          user.hasGreeted = true;
          await user.save();

          continue; // หลังทักเสร็จให้รอรอบถัดไปถึงจะเริ่มคุยกับ AI
        }
