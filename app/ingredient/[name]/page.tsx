...
export default function IngredientPage() {
    ...
    // Add states for animate-press control
    const [isBackAnimating, setIsBackAnimating] = useState(false);
    const [isChatbotAnimating, setIsChatbotAnimating] = useState(false);
    const [animatingShop, setAnimatingShop] = useState<string | null>(null);
    const [animatingPlaceId, setAnimatingPlaceId] = useState<string | null>(null);
    const [animatingRandomIngredientId, setAnimatingRandomIngredientId] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        service.nearbySearch(request, (results, status) => {
            if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
                console.log("✅ พบร้านค้า:", results.length);
                console.log("📦 รายการร้าน:", results);
                setPlaces(results);
            } else {
    ...
    // Navigation functions with animate-press
    const handleGoBack = () => {
        setIsBackAnimating(true);
        setTimeout(() => {
            gotoHome(previousMenuId, currentUserId);
    ...
    const handleChatbotClick = () => {
        ...
        setTimeout(() => {
            ...
            setIsChatbotAnimating(false);
        }, 300); // Match animate-press duration
    };

    const handleShopLinkClick = (shop: string, url: string) => {
        setAnimatingShop(shop);
        setTimeout(() => {
            window.open(url, '_blank');
            setAnimatingShop(null);
        }, 300); // Match animate-press duration
    };

    const handlePlaceClick = (placeId: string, placeName: string | undefined) => {
        setAnimatingPlaceId(placeId);
        setTimeout(() => {
            window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(placeName || '')}&query_place_id=${placeId}`, '_blank');
            setAnimatingPlaceId(null);
        }, 300); // Match animate-press duration
    };

    const handleRandomIngredientClick = (ingredientId: string, ingredientName: string) => {
        setAnimatingRandomIngredientId(ingredientId);
        ...
        setTimeout(() => {
            ...
            setAnimatingRandomIngredientId(null);
        }, 300); // Match animate-press duration
    };
        ...
        <div className="relative font-prompt min-h-screen bg-yellow overflow-x-hidden">
            ...
            <div className="absolute z-10 top-0 left-0 right-0 flex justify-between p-4 items-center w-full max-w-2xl mx-auto">
                ...
                <div
                    ...
                    className={`bg-white h-[50px] flex justify-center cursor-pointer transform transition duration-300 hover:scale-103 items-center w-[50px] rounded-full shadow-grey shadow-xl ${isBackAnimating ? "animate-press" : ''}`}
                >
                ...
                <div className="max-w-[360px] relative">
                    ...
                    <div className="absolute top-[-3.3rem] left-[18rem] -translate-x-1/2 md:left-[15rem] md:translate-x-0">
                        ...
                        <Image
                            ...
                            className={`mt-[3rem] animate-pulse animate-sizeUpdown cursor-pointer transform hover:scale-105 duration-300 ${isChatbotAnimating ? "animate-press" : ''}`}
                            src="/image%2069.png"
                            alt="Chatbot icon"
                            width={60}
                            height={60}
                            ...
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(ingredient.shopLinks).map(([shop, url]) => (
                                    <a
                                        key={shop}
                                        onClick={(e) => {
                                            e.preventDefault(); // Prevent default link behavior to allow animation
                                            handleShopLinkClick(shop, url);
                                        }}
                                        className={`
                                            ...
                                            ${shop === 'makro' ? 'bg-red-600' :
                                                shop === 'lotus' ? 'bg-white' :
                                                    shop === 'bigc' ? 'bg-[#a0c515]' :
                                                        'bg-gray-500' // Default background for other shops
                                            }
                                            ${animatingShop === shop ? "animate-press" : ''}
                                        `}
                                    >
                            ...
                            {places.length > 0 ? (
                                places.map((place) => {
                                    ...
                                    return (
                                        <div
                                            key={place.place_id}
                                            onClick={() => handlePlaceClick(place.place_id!, place.name)}
                                            className={`flex bg-white rounded-2xl border-2 border-[#C9AF90] px-[1rem] items-center flex-shrink-0 shadow-sm cursor-pointer hover:shadow-lg transition ${animatingPlaceId === place.place_id ? "animate-press" : ''}`}
                                        >
                                            <img
                            ...
                            randomIngredients.map((randomIngredient) => {
                                ...
                                return (
                                    <div
                                        ...
                                        onClick={() => handleRandomIngredientClick(randomIngredient._id, randomIngredient.name)}
                                        className={`flex flex-col items-center w-[130px] bg-white border-2 border-[#C9AF90] rounded-t-full shadow-sm cursor-pointer ${animatingRandomIngredientId === randomIngredient._id ? "animate-press" : ''}`}
                                    >
                                        <Image
