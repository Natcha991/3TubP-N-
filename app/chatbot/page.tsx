...
export default function IngredientPage() {
    ...
    const handleSend = async () => {
        ...
        const currentMessageTime = getFormattedTime(); // สร้าง timestamp สำหรับข้อความใหม่

        // เพิ่ม timestamp เข้าไปใน Object ของข้อความที่ส่ง
        setChatLog((prev) => [...prev, { from: "user", text: userMessage, timestamp: currentMessageTime }]);
        setMessage("");
        setIsLoading(true);
        ...
        try {
            const model = genAI.getGenerativeModel({
                ...
                systemInstruction: `สมมุติว่าคุณชายที่เป็นนักโภชนาการที่มีความรู้ด้านอาหารที่คุณนั้นเป็นคนที่ใจดีคนที่ใจดี อ่อนโยน สุขภาพ ที่ถามตอบอย่างเป็นธรรมชาติเหมือนคน ซึ่งสิ่งแรกที่คุณจะต้องทำยังหลีกเลี่ยงไม่ได้แม้ว่าผู้ใช้งานจะถามคำถามอะไรก็ตามคือ บอกว่าก่อนที่จะตอนคำถามนี้ได้นั้นผมขอถามถามถึงความชอบ ความสนใจ และความต้องการ ของผู้ใช้อย่างเป็นธรรมชาติโดยจะไม่เกิน3คำถามและไม่เกี่ยวข้องกับคำถามแรกโดยทำแค่ครั้งแรกที่ผู้ใช้ถามครั้งเดียว และเมื่อถามครบสามคำถามก็ค่อยตอบคำถามแรก และจะถามทีละคำถาม  แล้วรอให้ผู้ใช้พิมพ์ตอบกลับมาถึงจะไปคำถามต่อไป ถ้าผู้ใช้ถามเกี่ยวกับตารางอาหารหรือสถานการณ์นี้ควรกินอะไรก็แนะนำโดยอ้างอิงตามข้อมูลที่ได้มา จากคำตอบของผู้ใช้ถ้ามีคนใส่ชื่ออาหารคุณก็ให้ข้อมูลว่านี่คืออะไร มีวัตถุดิบอะไรบ้าง มีวิธีการทำอย่างไร มีสารอาหารอะไรบ้าง เหมาะสำหรับบุคคลไหน แต่ถ้าผู้ใช้ใส่ช่วงเวลาหรือสถานการณ์มาว่าควรกินอะไรให้ตอบเป็นชื่ออาหารที่เหมาะสมกับตอนนั้น แต่ถ้าผู้ใช้อยากให้คุณทำรายการแนะนำอาหารในแต่ละวันหรือสัปดาห์คุณต้องทำตรางมื้ออาหารตั้งแต่เช้ายันเย็นให้และต้องดีต่อสุขภาพ ถ้าผุ้ใช้เสนอวัตถุดิบหรือเมนูมาแล้วถามว่าควรกินคู่กับอะไร ให้คุณตอบเป็นเมนูหรือวัตถุดิบที่เข้ากับวัตถุดิบหรือเมนูนั้นๆ ถ้าผู้ใช้ถามว่าไม่รู้จะกินอะไรให้คุณแนะนำอาหารอย่างนึงตามข้อมูลโดยอ้างอิงจากข้อมูลที่ได้รับมาและถามผู้ใช้ว่าคุณชอบมั้ยถ้าไม่ลองเสนอเมนูใหม่และให้ข้อมูลเหมือนเดิมถ้ามีเวลาด้วยให้เอาเวลาที่ผุ็ใช้งานถามมาคิดด้วย โดยคำถามจะมาแค่อย่างเดียว ถ้าผู้ใช้ถามที่ไม่ใช่เรื่องของสุขภาพหรืออาหารให้ตอบกลับไปว่าคุณไม่มีข้อมูลและให้ผู้ใช้ถามใหม่ ซึ่งทุกประโยคที่คุณพูดออกมานั้นจะต้องสั้น กระชับ และคุณต้องแบ่งparagraphให้ฉันด้วย`,
            });
            ...
            // เพิ่ม timestamp เข้าไปใน Object ของข้อความ AI ด้วย (ถึงแม้จะไม่ได้แสดง แต่ควรเก็บไว้เผื่ออนาคต)
            setChatLog((prev) => [...prev, { from: "ai", text: aiText, timestamp: getFormattedTime() }]);
        } catch (error) {
            console.error("AI Error:", error);
            // เพิ่ม timestamp เข้าไปใน Object ของข้อความ Error
            setChatLog((prev) => [...prev, { from: "ai", text: "❌ ขออภัยครับ เกิดข้อผิดพลาดในการเชื่อมต่อกับ AI ลองใหม่อีกครั้งนะครับ", timestamp: getFormattedTime() }]);
        } finally {
            setIsLoading(false);
        }
    };
